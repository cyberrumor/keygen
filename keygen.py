...
def get_music(source, mixin, rhythm):
    ...
    for i in range(len(rhythm)):
        ...
        possible_quarter_notes = settings.possible_quarter_notes

        # collect appropriate number of melody rhythms
        melody_rhythm = []
        c = random.choice(possible_quarter_notes)
        flip = settings.flip
...
if __name__ == '__main__':
    # validate settings
    for index, entry in enumerate(settings.possible_quarter_notes):
        assert sum(entry) == 1024, f"settings.possible_quarter_notes[{index}] didn't sum to 1024: {sum(entry)=}"
    for index, entry in enumerate(settings.rhythms):
        assert sum(entry) == 4096, f"settings.rhythms[{index}] didn't sum to 4096: {sum(entry)=}"
        assert len(entry) <= 4, f"settings.rhythms[{index}] exceeded max length of 4: {len(entry)=}"
